    ...
    useEffect(() => {
        const fetchMembers = async () => {
            try {
                ...
                const res = await axios.get(
                    `http://localhost:8080/api/user-groups/group/${groupId}`
                );
                setMembers(res.data);
            } catch (err) {
        ...
        const fetchProposals = async () => {
            try {
                ...
                const res = await axios.get(
                    `http://localhost:8080/trip-proposals/group/${groupId}`
                );
                    ...
                    proposalsData.map(async (p) => {
                        try {
                            const approvalRes = await axios.get(
                                `http://localhost:8080/api/proposal-approvals/${user.userId}/${p.id}`
                            );
                            return { ...p, status: approvalRes.data.status };
                        } catch (err) {
                            console.warn(`No approval found for proposal ${p.id}`);
                            return { ...p, status: "PENDING" }; // fallback if no record
                        }
                    })
                );
            ...
            <div className="flex max-w-7xl mx-auto mt-24 gap-6 px-4">
                ...
                <motion.div className="flex-1 bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl relative p-8 flex flex-col">
                                ...
                                {proposals.map((p) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span
                                                className={`px-3 py-1 rounded-full text-xs font-bold ${p.status === "APPROVED"
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        className="absolute bottom-6 right-6 px-6 py-4 bg-gradient-to-r from-indigo-600 to-pink-600 text-white font-bold rounded-full shadow-lg hover:shadow-xl transition"
                    >
                        ➕ Add Trip
                    </motion.button>
                </motion.div>
